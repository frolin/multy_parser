module Imports
	class Xlsx
		...
		def find_range(page)
			Imports::Xlsx::FindRange.new(config: @config, spreadsheet: @spreadsheet, page: page).process!
		end

		def parse_all(ranges)
			ranges.each do |range|
				puts "Parse #{range[:page_name]}"
				Imports::Xlsx::ParseRange.new(config: @config, spreadsheet: @spreadsheet, range: range).process!
			end
		end
	end
end